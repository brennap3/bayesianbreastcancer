...
Proportion tests are standard statistical tools in frequentist statistics to show the association between categorical variables. Here they are re-implemented in a bayesian framework. 

They are used to answer the research question, 'Does irradiation (radiation treatment) effect recurrence of or non recurrence of breast cancer'. From the analysis there does appear to be a strong association between irradiation and recurrence of cancer, and non irradiation and non recurrence of cancer. 
...
It should be noted that all data are categorical in nature. We will read in the data, set the column names and output the first 5 rows to see the form of the data, we will also set the degree.malignant data attribute to a factor as this is ordinal data not numeric. 
...
It is clear from the bar chart there is a stronger association between irradiation and recurrence events as opposed to no irradiation and no recurrence events. The same data is shown below in a standard stacked bar chart form.
...
We will build a model which aims to show the proportions could not have come from equal proportions (i.e there is no association between them) as opposed to the proportions being unequal and a strong association exists between the groups (irradiation and recurrence events). This is equivalent to a bayesian re-interpretation of the proportions test.
...
There appears to be a strong association between irradiation and recurrence of cancer, and non irradiation and non recurrence of cancer.
